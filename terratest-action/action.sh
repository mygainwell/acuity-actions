...
function parse_inputs {
  ...
  if [ "${INPUT_AWS_ACCOUNTS_ID}" != "" ]; then
    aws_accounts_id=${INPUT_AWS_ACCOUNTS_ID}
  else
    echo "ERROR: AWS_ACCOUNTS Id stored in secrets maanger is required"
    exit 1
  fi
